...
- (void)drawLine {
    ...
    line.referenceColor = self.colorXaxisLabel;
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        if ([self.delegate respondsToSelector:@selector(minValueForLineGraph:)]) {
            minimumValue = [NSNumber numberWithFloat:[self.delegate minValueForLineGraph:self]];
        }
        else {
            minimumValue = [NSNumber numberWithInteger:[self calculateMinimumPointValue].integerValue];
        }
        
        NSNumber *maximumValue;
        if ([self.delegate respondsToSelector:@selector(maxValueForLineGraph:)]) {
            maximumValue = [NSNumber numberWithFloat:[self.delegate maxValueForLineGraph:self]];
        }
        else {
            maximumValue = [NSNumber numberWithInteger:[self calculateMaximumPointValue].integerValue];
        }
